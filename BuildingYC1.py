...
#    #swap with semi-annual payments: 
    PtTi = P(tau2, DF,T10) #array of PtTi's if semi annual
#    print('PtTi are', PtTi)
    #Swap_SA_3y = Swap(T0,T3,N,K3,tau2,PtTi)                 #using old script
#    Swap_SA_10y = Swap(T0,T10,N,K10,tau2,PtTi)             #using old script
#    Swap1_Test = Swap(T0,T1,N,K1,tau1,PtTi)                #using old script
...
#    print('test annual swap3 value', Swap3_Test)        #using old script
    
    #print('semi annual swap_3y value', Swap_SA_3y)      #using old script
...
#    print('test new annual swap3 value',newSwap3)
    ...
    newSwap_10y = NewSwap(T0,T10,N,K10,tau1,DF,t)
    print(newSwap_10y)
    print('test new semi annual swap 3y value',newSwap_SA_3y)
    print('difference semi annual swap 3y and annual swap 3y is:',np.abs(newSwap_SA_3y - newSwap3 ))
    print('diff semi annual swap should have diff with swap3:',-0.25*PtTi[6]+0.25*PtTi[0])           #should be difference with swap 3
    print('Discount factors of Python semi annual swap:', P4)
 
 #   print('Discount Factors are ',DF)

    PtT0_5 = 0.5*(PtT1+1)
    PtT1_5 = 0.5*(PtT1+PtT2)
    PtT2_5 = 0.5*(PtT2+PtT3)
    P_test = np.array([PtT0,PtT0_5,PtT1,PtT1_5,PtT2,PtT2_5,PtT3])
    P_maxdif = np.max(np.abs(P_test-P4))
    print('Discount factors of Hand semi annual swap:', P_test)
    print('P maxdif is',P_maxdif)
    V_annual_test = PtT3 - PtT0 + 0.01*K3*(0.25*PtT0+PtT1+ PtT2+0.75*PtT3)
    print(V_annual_test)
    print('sa test dif:',V_annual_test - newSwap_SA_3y)
    print(PtTi[6],PtT3,PtTi[0],PtT0)
    #newSwap_SA_2y,P5 = NewSwap(T0,T2,N,K2,tau3,DF,t)
    #print('test new semi annual swap 2y value, tau=0.4',newSwap_SA_2y)       #swap with tau = 0.4
    #print('annual swap 5 years is', NewSwap(T0,T5,N,K5,tau1,DF,t))
